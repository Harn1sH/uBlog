...
//set edit
app.put("/edit", uploadMiddleware.single("photo"), async (req, res) => {
  const { author, title, id, summary, content, photo } = req.body;
  const { token } = req.cookies;
  const postDoc = await postModel.findById(id);
  jwt.verify(token, secret, async (err, info) => {
    if (JSON.stringify(info.username) === JSON.stringify(author)) {
      if (req.file) {
        const { originalname, path } = req.file;
        const parts = originalname.split(".");
        const ext = parts[parts.length - 1];
        ...
        const result = await postModel.updateOne(
          { _id: id },
          { $set: { title, content, summary, cover: newPath } }
        );
        res.json(result);
      } else {
        const result = await postModel.updateOne(
